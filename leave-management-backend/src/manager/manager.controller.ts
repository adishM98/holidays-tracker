...
export class ManagerController {
  ...
  async getDashboardStats(@CurrentUser() user: User) {
    ...
    // Get team members
    const teamMembers = await this.employeesService.findSubordinates(
      manager.id,
    );
    ...
    return {
      ...
      activeTeamMembers: teamMembers.filter((member) => member.user?.isActive)
        .length,
  ...
  async getLeaveRequest(
    ...
    if (leaveRequest.employee.managerId !== manager.id) {
      throw new ForbiddenException(
        "You can only view requests from your team members",
      );
    }
  ...
  async approveLeaveRequest(
    ...
    if (leaveRequest.employee.managerId !== manager.id) {
      throw new ForbiddenException(
        "You can only approve requests from your team members",
      );
    }
  ...
  async rejectLeaveRequest(
    ...
    if (leaveRequest.employee.managerId !== manager.id) {
      throw new ForbiddenException(
        "You can only reject requests from your team members",
      );
    }

    return this.leavesService.rejectLeaveRequest(
      requestId,
      manager.id,
      rejectDto,
    );
  ...
  async getTeamLeaveSummary(
    ...
    const teamLeaveRequests = await Promise.all(
      teamMembers.map(async (member) => {
        ...
        return {
          ...
          approvedRequests: requests.requests.filter(
            (req) => req.status === LeaveStatus.APPROVED,
          ).length,
          pendingRequests: requests.requests.filter(
            (req) => req.status === LeaveStatus.PENDING,
          ).length,
          rejectedRequests: requests.requests.filter(
            (req) => req.status === LeaveStatus.REJECTED,
          ).length,
    ...
    return {
      ...
      overallStats: {
        totalRequestsPending: teamLeaveRequests.reduce(
          (sum, member) => sum + member.pendingRequests,
          0,
        ),
        totalDaysTaken: teamLeaveRequests.reduce(
          (sum, member) => sum + member.totalDaysTaken,
          0,
        ),
        averageDaysPerEmployee:
          teamLeaveRequests.length > 0
            ? Math.round(
                (teamLeaveRequests.reduce(
                  (sum, member) => sum + member.totalDaysTaken,
                  0,
                ) /
