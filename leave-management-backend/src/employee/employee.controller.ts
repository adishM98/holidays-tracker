...
export class EmployeeController {
  ...
  async getDashboard(@CurrentUser() user: User) {
    ...
    // Get leave balances
    const leaveBalances = await this.leavesService.getLeaveBalance(employee.id, currentYear);
    ...
    return {
      ...
      stats: {
        ...
        approvedThisYear: recentRequests.requests.filter(req => 
          req.status === 'approved' && 
  ...
  async getLeaveBalance(
    ...
    const balances = await this.leavesService.getLeaveBalance(employee.id, targetYear);
  ...
  async createLeaveRequest(
    ...
    return this.leavesService.createLeaveRequest(employee.id, createLeaveRequestDto);
  }
  ...
  async getLeaveCalendar(
    ...
    if (employee.departmentId) {
      ...
      teamCalendar = teamCalendar.filter(leave => leave.employeeId !== employee.id);
    }
  ...
  async getLeaveHistory(
    ...
    const summary = {
      ...
      cancelledRequests: filteredRequests.filter(req => req.status === 'cancelled').length,
